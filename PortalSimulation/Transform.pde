...
public class Transform {
  ...
  public void setRotCenter(BlockVec rotCenter) {
    this.rotCenter = rotCenter.clone(); //.add(0.5, 0.5, 0.5);
  }
  ...
  public BlockVec transformVec(BlockVec vec) {
    ...
    return vec.add(translation);
  }
  
  private void rotateVec(PVector relativeVec) {
    
    float transX = relativeVec.x;
    float transZ = relativeVec.z;
    
    relativeVec.x = rotYMatrix[0][0] * transX + rotYMatrix[0][1] * transZ;
    relativeVec.z = rotYMatrix[1][0] * transX + rotYMatrix[1][1] * transZ;
  }
